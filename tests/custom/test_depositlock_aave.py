...
def test_operation(
    ...
    # wait two days to prove deposit fail
    chain.sleep(3600*24*2)
    chain.mine(1)
    a = strategy.viewDepositLock({"from": gov})
    assert a == 1
    #this should revert if depositLock = 1
    #strategy.harvest({"from": gov}) testing proved failure here

    chain.sleep(3600*24*9)
    strategy.setDepositLock(0)
    chain.mine(1)
    strategy.harvest({"from": gov})
    chain.mine(1)
